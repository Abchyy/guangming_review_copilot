import type { EvidenceItem, Finding, Suggestion } from "@/lib/contracts/review";
import { isAuthoritativeSource } from "@/lib/server/quality/retrieval";
...
function mergePair(primary: DraftFinding, other: DraftFinding): DraftFinding {
  const ruleOwned = primary.evidence.some((item) => item.kind === "rule");
  return {
    ...primary,
    reason: other.reason.length > primary.reason.length ? other.reason : primary.reason,
    suggestion: ruleOwned ? primary.suggestion : primary.suggestion,
    evidence: mergeEvidence(primary.evidence, other.evidence),
    confidence: Math.max(primary.confidence, other.confidence),
    requires_verification: Boolean(primary.requires_verification && other.requires_verification),
  };
}
