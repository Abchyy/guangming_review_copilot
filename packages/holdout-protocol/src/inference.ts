import type { Finding, ReviewExecutionProvenance } from "@grc/contracts";
import { findingSchema } from "@grc/contracts";
import { readFileSync } from "node:fs";
import { z } from "zod";

import { HoldoutProtocolError, protocolErrorFrom } from "./errors";
import { sealedArtifactPath, writeSealedJson } from "./artifacts";
import {
...
import { rejectCallerWorkspaceOverride, assertCanonicalProcessCwd } from "./git-state";
import { sha256Canonical } from "./identity";
import type { InputPack } from "./input-pack";
import { claimForRole, type ResultClaim } from "./roles";
import {
...
import { snapshotFromProvenance, type CallRuntimeSnapshot } from "@grc/benchmark";
import { assertOfficialBenchmarkProvenance } from "@grc/providers";
...
export function assertOfficialInferenceProvenance(
  provenance: ReviewExecutionProvenance,
  fallback?: { used: boolean },
): void {
  if (fallback?.used) {
    throw new HoldoutProtocolError(
      "Official locked inference cannot seal a degraded fallback result",
    );
  }
  try {
...
export async function runBlindInference(options: BlindInferenceOptions): Promise<SealedPrediction> {
  ...
  for (const article of options.inputPack.articles) {
    ...
    if (officialPath) {
      assertOfficialInferenceProvenance(provenance, snapshot.pipeline.fallback);
    }
    articles.push({
