...
import { averageMetrics, evaluateReview, type BenchmarkMetrics } from "@grc/benchmark";
...
import { goldPackContentIdentity, type GoldPack } from "./gold-pack";
import { sha256Canonical, sha256File } from "./identity";
import {
  assertPredictionIntegrity,
  loadSealedPrediction,
  type SealedPrediction,
} from "./inference";
import { inputPackContentIdentity, type InputPack } from "./input-pack";
import {
...
import { assertNotOfficialLockedClaim, claimForRole, type ResultClaim } from "./roles";
...
export type HoldoutResultManifest = {
  ...
  evaluator_asset: "packages/benchmark/src/evaluate.ts";
...
export function runControlledEvaluation(options: ControlledEvaluationOptions): {
  ...
  const evaluator = freeze.assets.find((item) => item.path === "packages/benchmark/src/evaluate.ts");
  if (!evaluator) {
