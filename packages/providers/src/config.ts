...
export type FallbackMode = "rules_only" | "fail";

export function getFallbackMode(): FallbackMode {
  return process.env.REVIEW_FALLBACK_MODE?.trim() === "fail" ? "fail" : "rules_only";
}

export function specialistsEnabled(): boolean {
  return process.env.REVIEW_SPECIALISTS_ENABLED === "1";
}
