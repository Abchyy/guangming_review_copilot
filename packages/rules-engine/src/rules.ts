...
import { fieldText, paragraphIndexAt } from "./article-text";
import { RULE_VERSION } from "./versions";
...
function yearFromArticle(article: CanonicalArticle, fallback: number): number {
  const combined = `${article.title}\n${article.body}`
    .replace(/《[^》]*》/g, " ")
    .replace(/\d{4}\s*[－—\-至到]\s*\d{4}年/g, " ");
  const dated = combined.match(/(\d{4})年\s*\d{1,2}月\s*\d{1,2}日/);
  if (dated) {
    return Number(dated[1]);
  }
  return fallback;
}
